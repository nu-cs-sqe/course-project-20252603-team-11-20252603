...
- **TC13: path exists in map but intermediate territory is enemy-owned** ( :white_check_mark: )
...
- **TC20: source == destination (same territory object)** ( :white_check_mark: )
...
- **TC23: source and destination connected via two intermediate territories, all player-owned (3 hops)** ( :white_check_mark: )
...
- **TC55: source == destination (same territory object)** ( :white_check_mark: )
  - **State of the system**: Phase created
  - **Expected output**: IllegalArgumentException thrown
